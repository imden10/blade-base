...
<x-slot name="scripts">
    <script>
        let userIds = @json($user_ids);

        function checkUsersOnline() {
            $.ajax({
                url: '/admin/users/check-users-online',
                type: 'POST',
                data: {
                    _token: "{{ csrf_token() }}",
                    user_ids: userIds
                ...
                success: function(response) {
                    if(response.success){
                        for(let id in response.data){
                            if(response.data[id]){
                                $(".user-pic-component[data-id='"+id+"'] .uplabel").removeClass('bg-red-400');
                                $(".user-pic-component[data-id='"+id+"'] .uplabel").addClass('bg-green-400');
                            } else {
                                $(".user-pic-component[data-id='"+id+"'] .uplabel").addClass('bg-red-400');
                                $(".user-pic-component[data-id='"+id+"'] .uplabel").removeClass('bg-green-400');
                            }
                        }
                    }
                }
