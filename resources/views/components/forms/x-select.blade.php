    ...
    <div class="@if(isset($inline) && $inline) x-select-inline @endif">
        ...
        <div class="x-select-select">
            <div class="x-select-element @if(isset($error) && $error) x-select-error @endif">
                <button type="button" aria-expanded="false" onclick="xselectToogleList(event)">
                    <span>{{$options[$val]['text'] ?? ''}}</span>
                </button>
            </div>
            <div id="{{$componentId}}" class="x-select-dropdown hidden">
                @if(isset($searchable))
                    <div class="x-select-search">
                        <input type="text" oninput="xselectFilterList(event)">
                    </div>
                @endif
                        ...
                        @foreach($options as $id => $item)
                            <label data-text="{{$item['text']}}" class="@if((string)$val === (string)$id) x-select-selected @endif" title="{{$item['text']}}">
                                @if(isset($item['icon']))
                                    <i class="{{$item['icon']}}"></i>
                                @endif
                                <input type="radio" {{$attributes->except(['class','width','title','inline','searchable','options','val'])}} value="{{$id}}">
                                {{ $item['text'] }}
                            </label>
                        @endforeach
                    @endif
